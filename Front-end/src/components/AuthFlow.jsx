...
import { 
  ...
  CheckCircle, Eye, EyeOff, Sparkles, Check, AlertCircle 
} from "lucide-react";

export default function AuthFlow({ onAuthSuccess, onNavigateToPublic }) {
  const [isLogin, setIsLogin] = useState(true);
  const [selectedRole, setSelectedRole] = useState("Citizen"); // Citizen, Responder, NGO
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 md:p-8 relative overflow-hidden select-none">
      ...
      >
        ...
        {/* Left Informational Showcase (Dynamic based on selected role) */}
            ...
            {isSuccess ? (
              ...
              >
                ...
                <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                  <div className="flex gap-4">
                    <button 
                      ...
                      className={`text-sm font-extrabold pb-2 relative transition-colors ${
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  {!isLogin && selectedRole === "Responder" && (
                    <motion.div 
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: "auto" }}
                      className="space-y-1.5"
                    >
                      <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Credentials / License ID</label>
                      <div className="relative">
                        <Award className="w-4 h-4 text-slate-500 absolute left-3.5 top-3" />
                        <input 
                          type="text" 
                          placeholder="e.g. EMS-LICENSE-4491" 
                          value={license}
                  ...
                  >
                    {isLoading ? (
                      ...
                      <>
                        <span>{isLogin ? "Authenticate Session" : "Deploy Credentials"}</span>
